...
namespace EverlastingOverhaul.Common.RoguelikeMode.RoguelikeChange.Mechanic;
/// <summary>
...
public class Shield_GlobalItem : GlobalItem {
	...
	public static bool IsAShield(int type) => ModItemLib.Shield.Contains(type);
...
public class Shield_ModPlayer : ModPlayer {
	...
	public override void ResetEffects() {
		...
		Shield_MaxHealth = 0;
		Shield_CoolDown = ModUtils.CountDown(Shield_CoolDown);
	}
	...
	private void DamageShield(int damagevalue) {
		if (Shield_IsUp) {
			...
			ModUtils.CombatTextRevamp(Player.Hitbox, Color.BlueViolet, $"-{damageDone}", Main.rand.Next(30, 40), 5);
			if (Shield_Health <= 0) {
				Shield_IsUp = false;
				Player.DelBuff(Player.FindBuffIndex(ModContent.BuffType<Shield_ModBuff>()));
				Shield_CoolDown = ModUtils.ToSecond(10);
			}
		}
	}
}
