...
namespace EverlastingOverhaul.Common.RoguelikeMode.RoguelikeChange.NPCsOverhaul.Bosses.EyeofCuthulu;
public class EyeofCuthulu : NPCReworkerFSM {
	...
	public override bool PreDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
		...
		if (states.currentState is EoC_Projectile state) {
			...
			ModUtils.DrawPrettyStarSparkle(1f, SpriteEffects.None, npc.Center + npc.rotation.ToRotationVector2() * 64 - screenPos, Color.Purple, Color.Red, progress, 0, 0.5f, 0.5f, 1f, npc.rotation, Vector2.One * 4, Vector2.One);
		}
		if (states.currentState is EoC_DashAndFire state2) {
			float progress = (float)state2.counter / 60;

			ModUtils.DrawPrettyStarSparkle(1f, SpriteEffects.None, npc.Center + npc.rotation.ToRotationVector2() * 128 - screenPos, Color.Purple, Color.Red, progress, 0, 0.5f, 0.5f, 1f, npc.rotation + MathHelper.TwoPi * progress, Vector2.One * 4, Vector2.One);
		}
...
public class LingeringEvilFlames : ModProjectile {
	...
	public override bool PreDraw(ref Color lightColor) {
		default(LingeringEvilFlamesQuad).Draw(Projectile.Center - Main.screenPosition, Projectile.rotation, new Vector2(150 * Projectile.scale), randomOffset, MathHelper.Lerp(1, 0, ModUtils.InExpo((float)Projectile.timeLeft / 60, 11f)));
...
//		{
//			ModUtils.Push(ref points, EoC.Center);
//			ModUtils.Push(ref rots, EoC.rotation);		
//			index++;
//		}
...
//			if(ModUtils.Collision_PointAB_EntityCollide(targetHitbox,points[i],points[i + 1]))
//				return true;
	...
	public override void OnStateUpdate(CommonNPCInfo info) {
		...
		if (counter == 60)
			if (reDashCounter < 2)
				...
				ChangeState(StateType<EoC_SummonSpin>());
...
public class EoC_DashTeleport : AIState {
	...
	public override void OnStateUpdate(CommonNPCInfo info) {
		npc.velocity = npc.rotation.ToRotationVector2() * 30 * ModUtils.InExpo(1f - counter / 60f, counter / 60f * 2f);
		if (counter == 60)
			if (tpCount < 6)
				ChangeState(StateType<EoC_RandomTeleport>());
			else
