...
namespace EverlastingOverhaul.Common.RoguelikeMode.RoguelikeChange.NPCsOverhaul.Bosses.KingSlime;
internal class KingSlime : NPCReworker {
	...
	public override void ReworkedAI(ref NPC npc, Player target) {
		...
		currentScale = Vector2.Lerp(Vector2.One, bouncyScale, ModUtils.EaseOutBounce(Delay / 40f)) + npc.velocity * 0.025f;
		...
		if (Delay == 0)
			switch (state) {
				...
				case State.SlamJump: {
						...
						currentScale.Y = 1f.ClampedLerp(0.5f, ModUtils.EaseOutBounce(Counter / 100f));
						Counter++;
...
public class SlimeKingRubyBolt : ModProjectile {
	...
	public override void AI() {
		...
		ModUtils.Push(ref oldPos, Projectile.Center);
		...
		switch (Projectile.ai[1]) {
			...
			case 2: {
					if (Projectile.timeLeft <= 30) {
						progress = 1f.ClampedLerp(0f, ModUtils.InOutExpo(Projectile.timeLeft / 30f, 3f));
	...
	public override bool PreDraw(ref Color lightColor) {
		...
		ModUtils.DrawPrettyStarSparkle(1f, SpriteEffects.None, startingPos - Main.screenPosition, Color.Red, Color.Red, (Projectile.timeLeft - 280f) / 20f, 0.2f, 1, 0.5f, 0f, Projectile.rotation, Vector2.One * 5f, Vector2.One);
	...
	public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
		...
		if (Projectile.ai[1] == 2 && progress > 0.25)
			for (float i = MathHelper.TwoPi; i > 0; i -= MathHelper.TwoPi / numOfLasers) {
				...
				if (ModUtils.Collision_PointAB_EntityCollide(targetHitbox, Projectile.Center, Projectile.Center + (i + lasersRotation).ToRotationVector2() * 4000))
					return true;
...
public class KingSlimeCrown : ModNPC {
	...
	public override void PostDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor) {
		...
		ModUtils.DrawPrettyStarSparkle(1f, SpriteEffects.None, NPC.Center - screenPos, Color.IndianRed, Color.Red, MathF.Sin((float)Main.timeForVisualEffects * 0.1f), 0, 1, 1, 2, MathHelper.PiOver4 + MathF.Sin((float)Main.timeForVisualEffects * 0.1f), Vector2.One * 5, Vector2.One);
	}
...
public class KingSlimeMinion : ModNPC {
	...
	public override string Texture => ModUtils.GetVanillaTexture<NPC>(NPCID.BlueSlime);
	public override void SetDefaults() {
		NPC.CloneDefaults(NPCID.BlueSlime);
