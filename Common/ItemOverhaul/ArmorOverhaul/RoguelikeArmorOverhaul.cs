...
namespace EverlastingOverhaul.Common.ItemOverhaul.ArmorOverhaul;
class RoguelikeArmorOverhaul : GlobalItem {
	...
	public override void UpdateEquip(Item item, Player player) {
		...
		var def = ArmorLoader.GetArmorPieceInfo(type);
		if (def != null) {
...
public class RoguelikeArmorPlayer : ModPlayer {
	...
	public void SafeAddArmorSet(string armorSetName) {
		var set = ArmorLoader.GetModArmor(armorSetName);
	...
	public override void UpdateEquips() {
		...
		if (Player.ItemAnimationActive && Player.itemAnimation == Player.itemAnimationMax / 2) {
			var vel = (Main.MouseWorld - Player.Center).SafeNormalize(Vector2.Zero);
