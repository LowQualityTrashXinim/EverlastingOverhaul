...
public class PearlwoodArmorPlayer : PlayerArmorHandle {
	...
	public override void Armor_ResetEffects() {
		pearlWoodArmorCD = ModUtils.CountDown(pearlWoodArmorCD);
	}
	...
	private void OnHitNPC_PearlWoodArmor(NPC npc) {
		if (pearlWoodArmorCD <= 0) {
			...
			for (int i = 0; i < 6; i++) {
				var pos = npc.Center + new Vector2(0, -20).Vector2DistributeEvenly(6, 360, i) * 10;
				var vel = npc.Center - pos;
			...
			pearlWoodArmorCD = ModUtils.ToSecond(4);
		}
	}
}
