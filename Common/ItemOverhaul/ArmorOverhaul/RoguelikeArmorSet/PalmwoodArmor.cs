...
public class PalmwoodArmorModPlayer : ModPlayer {
	...
	public override void UpdateEquips() {
		if (SandJump) {
			if (Player.justJumped) {
				for (int i = 0; i < 4; i++) {
					var vec = new Vector2(-Player.velocity.X, Player.velocity.Y).Vector2RotateByRandom(20).LimitedVelocity(Main.rand.NextFloat(2, 3));
	...
	public override void OnHitByNPC(NPC npc, Player.HurtInfo hurtInfo) {
		if (Temporary_SandArmor) {
			Player.AddBuff<SandArmorBuff>(ModUtils.ToSecond(5));
		}
	}
