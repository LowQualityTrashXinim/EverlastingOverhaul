...
public class TungstenArmorPlayer : PlayerArmorHandle {
	...
	public override void Armor_UpdateEquipsSet() {
		...
		if (!Player.HasBuff<TungstenEmpowerment>()) {
			Player.AddBuff<TungstenEmpowerment>(ModUtils.ToSecond(10));
		}
	}
	public int Rotate = 0;
}
public class TungstenEmpowerment : ModBuff {
	...
	public override void Update(Player player, ref int buffIndex) {
		var modplayer = player.GetModPlayer<TungstenArmorPlayer>();
		if (player.buffTime[buffIndex] <= 0) {
			modplayer.Rotate = ModUtils.Safe_SwitchValue(modplayer.Rotate, 5);
		}
