...
public class CactusArmorPlayer : PlayerArmorHandle {
	...
	public override void Armor_ResetEffects() {
		CactusArmorCD = ModUtils.CountDown(CactusArmorCD);
	}
	...
	private void OnHitEffect_CactusArmor(Entity entity) {
		if (CactusArmorCD <= 0) {
			...
			var AbovePlayer = Player.Center + new Vector2(Main.rand.NextFloat(-500, 500), -1000);
		...
		for (int i = 0; i < 8; i++) {
			var vec = Vector2.One.Vector2DistributeEvenly(8, 360, i);
