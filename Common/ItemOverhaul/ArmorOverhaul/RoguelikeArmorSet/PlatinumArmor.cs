...
namespace EverlastingOverhaul.Common.ItemOverhaul.ArmorOverhaul.RoguelikeArmorSet;
...
public class PlatinumArmorModPlayer : PlayerArmorHandle {
	...
	private void MultiplyDamage(ref NPC.HitModifiers modifiers) {
		...
		var projectile = Main.projectile[GemProjectileWhoAmI];
	...
	public override void Armor_OnHitByNPC(NPC target, Player.HurtInfo hurtInfo) {
		Player.AddBuff(ModContent.BuffType<PlatinumDefense>(), ModUtils.ToSecond(4));
...
public class GemCollectible : ModProjectile {
	...
	public override void AI() {
		for (int i = 0; i < dustAmount / 10; i++) {
			var dustEffect = Dust.NewDustDirect(Projectile.Center, 0, 0, dust);
		...
		if (Projectile.ai[1] == 1) {
			...
			var modplayer = player.GetModPlayer<PlatinumArmorModPlayer>();
	...
	public override bool PreDraw(ref Color lightColor) {
		...
		var texture = TextureAssets.Item[DrawItemIDData].Value;
