...
namespace EverlastingOverhaul.Common.Graphics.Primitives;
	...
	public void Draw(Vector2[] positions, Color[] colors = default, Vector2[] size = default, float rotation = 0, Vector2 rotationCenter = default) {
		switch (Shape) {
			...
			case PrimitiveShape.Quad:
				...
				vertices[1].Position = (positions[0] + new Vector2(size[0].X * 0.5f, (float)-size[0].Y * 0.5f)).RotatedBy(rotation, rotationCenter);
				vertices[2].Position = (positions[0] + new Vector2((float)-size[0].X * 0.5f, size[0].Y * 0.5f)).RotatedBy(rotation, rotationCenter);
				vertices[3].Position = (positions[0] + new Vector2(size[0].X * 0.5f, size[0].Y * 0.5f)).RotatedBy(rotation, rotationCenter);
