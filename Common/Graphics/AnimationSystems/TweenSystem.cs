...
namespace EverlastingOverhaul.Common.Graphics.AnimationSystems;
	...
	public void Update() {
		...
		switch (easeType) {
			...
			case TweenEaseType.InSine:
				currentProgressPercentage = ModUtils.InSine(currentDuration / endDuration);
				break;
			case TweenEaseType.OutSine:
				currentProgressPercentage = ModUtils.OutSine(currentDuration / endDuration);
				break;
			case TweenEaseType.InExpo:
				currentProgressPercentage = ModUtils.InExpo(currentDuration / endDuration, 11f);
				break;
			case TweenEaseType.OutExpo:
				currentProgressPercentage = ModUtils.OutExpo(currentDuration / endDuration, 11f);
				break;
		}
		if (pingpongEnabled)
