...
namespace EverlastingOverhaul.Contents.Items.Weapon.RangeSynergyWeapon.TundraBow;
	...
	public override void UpdateEquips() {
		if (SnowStormCounter > 0 && SnowStormItemSource != null) {
			...
			for (int i = 0; i < 4; i++) {
				...
				projectile.timeLeft = ModUtils.ToSecond(2.5f);
			}
		}
	}
	...
	public override void OnHitNPCWithProj(Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (proj.Check_ItemTypeSource(ModContent.ItemType<TundraBow>())) {
			...
			if (proj.type == ProjectileID.SnowBallFriendly) {
				target.AddBuff<TundraHypothermia>(ModUtils.ToSecond(2));
			}
		}
	}
}
