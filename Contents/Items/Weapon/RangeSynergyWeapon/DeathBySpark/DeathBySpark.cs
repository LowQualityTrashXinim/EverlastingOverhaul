...
namespace EverlastingOverhaul.Contents.Items.Weapon.RangeSynergyWeapon.DeathBySpark
{
	...
	internal class SparkFlare : ModProjectile {
		...
		public override void AI() {
			...
			int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, OppositeVelocity + Main.rand.NextVector2Circular(1f, 1f), ProjectileID.WandOfSparkingSpark, (int)(Projectile.damage * 0.65f), 1f, Projectile.owner);
			...
			if (SynergyBonus_System.Check_SynergyBonus(ModContent.ItemType<DeathBySpark>(), ItemID.AleThrowingGlove) && ++Projectile.ai[1] >= 30) {
				Projectile.ai[1] = 0;
				Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, OppositeVelocity.Vector2RotateByRandom(15) * 4f, ProjectileID.Ale, (int)(Projectile.damage * 1.25f), 3f, Projectile.owner);
			}
		}
