...
namespace EverlastingOverhaul.Contents.Items.Weapon.RangeSynergyWeapon.SnowballCannonMK2;
internal class SnowballCannonMK2 : SynergyModItem {
	...
	public override void SynergyShoot(Player player, PlayerSynergyItemHandle modplayer, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, out bool CanShootItem) {
		...
		if (player.altFunctionUse == 2) {
			...
			player.AddBuff<SnowballCannonMK2Projectile_CoolDown>(ModUtils.ToSecond(1));
			return;
		}
		int amount = 15;
		float scalerMax = 1;
...
public class SnowballCannonMK2Projectile : ModProjectile {
	...
	public override void AI() {
		...
		for (int i = 0; i < 4; i++) {
			...
			dust.noGravity = true;
			dust.position = ModUtils.NextPointOn2Vector2(player.Center, player.Center.PositionOFFSET((Projectile.rotation + (player.direction > 0 ? 0 : MathHelper.Pi)).ToRotationVector2(), 82)) + Main.rand.NextVector2Circular(8, 8);
