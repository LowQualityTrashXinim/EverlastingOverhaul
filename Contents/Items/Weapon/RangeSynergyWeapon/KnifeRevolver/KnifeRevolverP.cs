...
namespace EverlastingOverhaul.Contents.Items.Weapon.RangeSynergyWeapon.KnifeRevolver
{
	internal class KnifeRevolverP : ModProjectile {
		...
		public override void OnKill(int timeLeft) {
			...
			foreach (var npc in npclist) {
				player.StrikeNPCDirect(npc, npc.CalculateHitInfo(Projectile.damage, ModUtils.DirectionFromPlayerToNPC(Projectile.Center.X, npc.Center.X), knockBack: 5f));
			}
		}
	}
}
