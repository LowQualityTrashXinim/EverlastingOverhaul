...
namespace EverlastingOverhaul.Contents.Items.Weapon.MeleeSynergyWeapon.EnergyBlade;
internal class EnergyBlade : SynergyModItem {
	...
	public override void SynergyShoot(Player player, PlayerSynergyItemHandle modplayer, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, out bool CanShootItem) {
		if (SynergyBonus_System.Check_SynergyBonus(Type, ItemID.Code1)) {
			if (player.GetModPlayer<EnergyBlade_ModPlayer>().EnergyBlade_Code1_Energy > 0) {
				int counter = player.GetModPlayer<EnergyBlade_ModPlayer>().EnergyBlade_Code1_Energy;
				for (int i = 0; i < counter; i++) {
					Vector2 lerp = velocity.Vector2DistributeEvenly(counter, 90, i) * 5f;
				...
				player.GetModPlayer<EnergyBlade_ModPlayer>().EnergyBlade_Code1_Energy = 0;
			}
...
public class EnergyBladeProjectile : ModProjectile {
	...
	public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
		...
		if (SynergyBonus_System.Check_SynergyBonus(Type, ItemID.Code1)) {
			if (Projectile.ai[0] == 1) {
				Vector2 Toplayer = (player.Center - target.Center).SafeNormalize(Vector2.Zero);
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Toplayer * 6f, ModContent.ProjectileType<EnergyBladeEnergyBallProjectile>(), 0, 0, Projectile.owner);
			}
		}
	}
	Vector2 data;
	...
	public override void AI() {
		...
		ModUtils.ProjectileSwordSwingAI(Projectile, player, data, 2);
	...
	public override void ModifyDamageHitbox(ref Rectangle hitbox) {
		if (Projectile.ai[0] == 1) {
			ModUtils.ModifyProjectileDamageHitbox(ref hitbox, player, outrotation, (int)(Projectile.width * 1.5f), (int)(Projectile.height * 1.5f), 140f);
			return;
		}
		ModUtils.ModifyProjectileDamageHitbox(ref hitbox, player, Projectile.width, Projectile.height);
	}
...
class EnergyBladeEnergyBallProjectile : ModProjectile {
	...
	public override void AI() {
		...
		if (ModUtils.CompareSquareFloatValue(Projectile.Center, Main.player[Projectile.owner].Center, 20 * 20)) {
			Main.player[Projectile.owner].GetModPlayer<EnergyBlade_ModPlayer>().EnergyBlade_Code1_Energy++;
			Projectile.Kill();
		}
	}
}
public class EnergyBlade_ModPlayer : ModPlayer {
