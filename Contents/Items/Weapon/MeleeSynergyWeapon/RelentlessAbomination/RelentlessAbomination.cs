...
namespace EverlastingOverhaul.Contents.Items.Weapon.MeleeSynergyWeapon.RelentlessAbomination {
	...
	class RA_ZombieArm : ModProjectile {
		public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.ZombieArm);
		public override void SetDefaults() {
			Projectile.width = 38;
		...
		public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers) {
			if (target.HasBuff<RA_Rotting>()) {
				modifiers.SourceDamage += .1f;
			}
		}
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			if (!target.HasBuff<RA_Rotting>()) {
				target.AddBuff<RA_Rotting>(ModUtils.ToSecond(Main.rand.Next(3, 8)));
			}
		}
	}
	...
	class RA_AntlionClaw : ModProjectile {
	...
	class RA_BoneSword : ModProjectile {
		public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.BoneSword);
		public override void SetDefaults() {
	...
	class RA_BatBat : ModProjectile {
		public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.BatBat);
		public override void SetDefaults() {
	...
	class RA_PurpleClubberfish : ModProjectile {
	...
	class RA_BeeKeeper : ModProjectile {
