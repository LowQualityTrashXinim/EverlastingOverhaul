...
namespace EverlastingOverhaul.Contents.Items.Weapon.MeleeSynergyWeapon.SakuraKatana;
internal class SakuraKatana : SynergyModItem {
	...
	public override void OnHitNPCSynergy(Player player, PlayerSynergyItemHandle modplayer, NPC target, NPC.HitInfo hit, int damageDone) {
		...
		if (strikecount > 30) {
			sakuraplayer.CherryBlossomAura_CoolDown = ModUtils.ToSecond(20);
			sakuraplayer.CherryBlossomAura_Duration = ModUtils.ToSecond(30);
...
public class SakuraKatana_ModPlayer : ModPlayer {
	...
	public override void ResetEffects() {
		DelayHit = ModUtils.CountDown(DelayHit);
		if (CherryBlossomAura_Duration > 0) {
			...
			CherryBlossomAura_Duration = ModUtils.CountDown(CherryBlossomAura_Duration);
			DataStorer.ActivateContext(CherryBlossomAura_Position, "Synergy_Sakura");
		}
		else {
			CherryBlossomAura_CoolDown = ModUtils.CountDown(CherryBlossomAura_CoolDown);
		}
		...
		if (--SakuraArt_CountDown <= 0) {
			SakuraArt = ModUtils.CountDown(SakuraArt);
			SakuraArt_CountDown = ModUtils.ToSecond(7);
		}
	}
	public override void UpdateEquips() {
		...
		if (CherryBlossomAura_Duration % 20 == 0) {
			...
			for (int i = 0; i < amount; i++) {
				...
				Vector2 pos = CherryBlossomAura_Position + Main.rand.NextVector2Circular(500, 500);
				Projectile proj = Projectile.NewProjectileDirect(Player.GetSource_FromThis(), pos, vel, ModContent.ProjectileType<SakuraLeaf_Projectile_1>(), damage, 0, Player.whoAmI);
				proj.penetrate = 1;
