...
namespace EverlastingOverhaul.Contents.Items.Weapon
{
	public class FairFrozen : ModItem {
		...
		public override void OnHitNPC(Player player, NPC target, NPC.HitInfo hit, int damageDone) {
			if (target.HasBuff(BuffID.Frostburn)) {
				...
				target.AddBuff(BuffID.Frostburn, ModUtils.ToSecond(Main.rand.Next(1, 5)));
			}
		}
	}
	...
	public class FairFrozenMinion : ModProjectile {
		public override string Texture => ModUtils.GetTheSameTextureAsEntity<FairFrozen>();
		public override void SetStaticDefaults() {
		...
		public override void AI() {
			...
			AliveTimer = ModUtils.Safe_SwitchValue(AliveTimer, 1000);
			...
			if (npc == null) {
				...
				if (AICooldown <= 0) {
					...
					AICooldown = ModUtils.CountDown((int)AICooldown);
				}
