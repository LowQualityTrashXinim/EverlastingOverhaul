...
namespace EverlastingOverhaul.Contents.Items.Weapon.MagicSynergyWeapon.LaserSniper;
internal class LaserSniper : SynergyModItem {
	...
	public override void SynergyShoot(Player player, PlayerSynergyItemHandle modplayer, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, out bool CanShootItem) {
		if (player.altFunctionUse == 2) {
			...
			var proj = Projectile.NewProjectileDirect(source, position.PositionOFFSET(velocity, 90), velocity, type, damage, knockback, player.whoAmI);
			if (!player.GetModPlayer<LaserSniperPlayer>().ManaMissing) {
				proj.penetrate = 3;
				proj.maxPenetrate = 3;
			}
