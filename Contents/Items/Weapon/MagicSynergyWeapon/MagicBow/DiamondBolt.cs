...
namespace EverlastingOverhaul.Contents.Items.Weapon.MagicSynergyWeapon.MagicBow
{
	internal class DiamondBolt : ModProjectile {
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			...
			if (player.ownedProjectileCounts[ModContent.ProjectileType<DiamondGemP>()] <= 50) {
				var Rotate = Projectile.Center + Main.rand.NextVector2CircularEdge(300, 300) + Main.rand.NextVector2Circular(30, 30) * (10 + Main.rand.NextFloat(5));
		...
		public override void AI() {
			...
			if (RicochetOff(Main.player[Projectile.owner], out var pos2)) {
				...
				for (int i = 0; i < 15; i++) {
					var ReverseVelSpread = -Projectile.velocity * 2 + Main.rand.NextVector2Circular(5f, 5f);
		...
		public bool RicochetOff(Player player, out Vector2 Pos2) {
			var list = player.GetModPlayer<DiamondGemProjectilePlayerTracker>().list_DiamondProjectile;
	...
	public class DiamondGemProjectilePlayerTracker : ModPlayer {
		...
		public override void ResetEffects() {
			for (int i = list_DiamondProjectile.Count - 1; i >= 0; i--) {
				var projectile = list_DiamondProjectile[i];
				if (projectile == null) {
