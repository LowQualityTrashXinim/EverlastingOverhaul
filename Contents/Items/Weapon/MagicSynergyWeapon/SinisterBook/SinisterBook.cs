...
namespace EverlastingOverhaul.Contents.Items.Weapon.MagicSynergyWeapon.SinisterBook;
...
internal class SinisterBolt : ModProjectile {
	...
	public override void AI() {
		if (CountMain == 0) {
			...
			DirectionFace = Main.player[Projectile.owner].direction == 1;
		}
		CountMain++;
		if (CountMain == 10)
	...
	public override void OnKill(int timeLeft) {
		if (SynergyBonus_System.Check_SynergyBonus(ModContent.ItemType<SinisterBook>(), ItemID.DemonScythe)) {
			Main.player[Projectile.owner].GetModPlayer<SinisterBook_ModPlayer>().SinisterBook_DemonScythe_Counter++;
		}
...
public class SinisterBook_ModPlayer : ModPlayer {
