...
namespace EverlastingOverhaul.Contents.Items.Weapon.MagicSynergyWeapon.Swotaff
{
	...
	public abstract class SwotaffProjectile : ModProjectile {
		...
		public override void AI() {
			...
			if (Projectile.ai[0] == 1 || Projectile.ai[0] == -1) {
				player.heldProj = Projectile.whoAmI;
				ModUtils.ProjectileSwordSwingAI(Projectile, player, PosToGo, (int)Projectile.ai[0], 150);
				return;
			}
		...
		public override void ModifyDamageHitbox(ref Rectangle hitbox) {
			...
			ModUtils.ModifyProjectileDamageHitbox(ref hitbox, Main.player[Projectile.owner], Projectile.width, Projectile.height);
		}
		...
		private void SpinAroundPlayer(Player player) {
			...
			//percentDone = ModUtils.InExpo(percentDone);
			...
			if (item.ModItem is SwotaffGemItem) {
				if (SynergyBonus_System.Check_SynergyBonus(Player.HeldItem.type, ItemID.Spear) && Player.altFunctionUse != 2) {
					if (Swotaff_Spear_Counter < 2) {
						...
						Vector2 cirRanPos = ModUtils.SpawnRanPositionThatIsNotIntoTile(position, 30, 90, velocity.ToRotation());
