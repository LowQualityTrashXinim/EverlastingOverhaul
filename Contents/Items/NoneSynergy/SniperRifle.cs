...
public class SniperRifle : ModItem {
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
		if (player.GetModPlayer<LegacySniperRiflePlayer>().Counter >= 120 + player.itemAnimationMax) {
			...
			var newPos = position.PositionOFFSET(velocity, 80);
			for (int i = 0; i < 500; i++) {
				var dust = Dust.NewDustDirect(newPos, 0, 0, DustID.GemDiamond);
				dust.noGravity = true;
				if (i % 2 == 0) {
			...
			for (int i = 0; i < 200; i++) {
				...
				var rotate = Main.rand.NextVector2CircularEdge(10, 3.5f).RotatedBy(velocity.ToRotation() + MathHelper.PiOver2);
		...
		var offset = HoldoutOffset() ?? new Vector2(-30, -7);
		var proj = Projectile.NewProjectileDirect(source, position + (offset * player.direction).RotatedBy(velocity.ToRotation()), velocity, ProjectileID.BulletHighVelocity, damage, knockback, player.whoAmI);
...
public class LegacySniperRiflePlayer : ModPlayer {
	...
	public override void OnHitNPCWithProj(Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (Counter >= 120 + Player.itemAnimationMax && proj.Check_ItemTypeSource(ModContent.ItemType<SniperRifle>())) {
			target.Center.LookForHostileNPC(out var npclist, 500);
			if (npclist.Count < 1) {
				return;
			}
			foreach (var npc in npclist) {
				...
				npc.AddBuff<CompletelyShatter>(ModUtils.ToSecond(20));
			}
			for (int i = 0; i < 150; i++) {
