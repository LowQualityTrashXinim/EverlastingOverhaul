...
namespace EverlastingOverhaul.Contents.Items.NoneSynergy.GuideToMasterNinja
{
	internal class GuideToMasterNinja : ModItem {
		...
		public override void ModifyTooltips(List<TooltipLine> tooltips) {
			...
			if (player.GetModPlayer<PlayerNinjaBook>().NinjaWeeb) tooltips.Add(new TooltipLine(Mod, "", $"[i:{ItemID.NinjaHood}][i:{ItemID.NinjaShirt}][i:{ItemID.NinjaPants}]Increase melee attack is faster by 15% and increase melee damage by 25%"));
			...
			if (HasThrowingKnife
				&& HasPoisonedKnife
				&& HasFrostDaggerFish
				&& HasBoneDagger) tooltips.Add(new TooltipLine(Mod, "", ThrowingKnife + PoisonedKnife + FrostDaggerfish + BoneDagger + "You can throw even faster"));
			if (HasThrowingKnife && HasPoisonedKnife) tooltips.Add(new TooltipLine(Mod, "", ThrowingKnife + PoisonedKnife + " You will sometime throw 1 of 2 knife and have a chance to spawn itself ontop of the enemy, Increases damage by 10"));
			else if (HasThrowingKnife) tooltips.Add(new TooltipLine(Mod, "", ThrowingKnife + "You will sometime throw throwing knife, damage +5"));
			else if (HasPoisonedKnife) tooltips.Add(new TooltipLine(Mod, "", PoisonedKnife + "You will sometime throw poisoned throwing knife, damage +5"));
			if (HasFrostDaggerFish) tooltips.Add(new TooltipLine(Mod, "", FrostDaggerfish + "Attack now inflict FrostBurn and you sometime throw FrostDaggerFish, damage +5"));
			if (HasBoneDagger) tooltips.Add(new TooltipLine(Mod, "", BoneDagger + "Attack now inflict OnFire! and you sometime throw BoneDagger, damage +5"));
			if (player.HasItem(ItemID.Katana)) tooltips.Add(new TooltipLine(Mod, "KatanaPower", $"[i:{ItemID.Katana}] Increase sword size and damage by 50% and melee speed by 35%"));
		}
	...
	internal class PlayerNinjaBook : ModPlayer {
		...
		public override void ResetEffects() {
			...
			if (Player.head == ArmorIDs.Head.NinjaHood && Player.body == ArmorIDs.Body.NinjaShirt && Player.legs == ArmorIDs.Legs.NinjaPants) NinjaWeeb = true;
			else NinjaWeeb = false;
		}
		public override void UpdateEquips() {
			if (!GuidetoMasterNinja) return;
			...
			if (++TimerForUltimate >= 60) {
				...
				if (Player.ownedProjectileCounts[ModContent.ProjectileType<ThrowingKnifeCustom>()] < 1) for (int i = 0; i < 8; i++)
				...
				if (Player.ownedProjectileCounts[ModContent.ProjectileType<ShurikenCustom>()] < 1) for (int i = 0; i < 5; i++)
		...
		public override bool Shoot(Item item, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
			if (GuidetoMasterNinja) if (++GTMNcount >= GTMNlimitCount) {
					var GTMNcontain = new List<int>();
					int StaticDamage = 10;
					if (NinjaWeeb) StaticDamage = (int)(StaticDamage * 1.2f);
					if (Player.HasItem(ItemID.ThrowingKnife)
						&& Player.HasItem(ItemID.PoisonedKnife)
						&& Player.HasItem(ItemID.FrostDaggerfish)
						&& Player.HasItem(ItemID.BoneDagger)) GTMNcount++;
		...
		public override void OnHitNPCWithProj(Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
			if (!GuidetoMasterNinja) return;
			if (Player.HasItem(ItemID.FrostDaggerfish)) target.AddBuff(BuffID.Frostburn, 150);
			if (Player.HasItem(ItemID.BoneDagger)) target.AddBuff(BuffID.OnFire, 150);
			if ((Player.HasItem(ItemID.Shuriken) || Player.HasItem(ItemID.ThrowingKnife) || Player.HasItem(ItemID.PoisonedKnife)) && Main.rand.NextBool(10)) {
				...
				if (Player.HasItem(ItemID.BoneDagger)) NinjaBag.Add(ProjectileID.BoneDagger);
				if (Player.HasItem(ItemID.FrostDaggerfish)) NinjaBag.Add(ProjectileID.FrostDaggerfish);
				var SpawnProjPos = target.Center.Add(0, 200);
				for (int i = 0; i < 12; i++) {
		...
		public override void ModifyItemScale(Item item, ref float scale) {
			if (GuidetoMasterNinja && item.type == ItemID.Katana) scale += .5f;
		}
		public override void ModifyWeaponDamage(Item item, ref StatModifier damage) {
			...
			if (item.type == ItemID.Shuriken
				...
				damage.Flat += 5f;
				if (Player.HasItem(ItemID.BoneDagger)) damage.Flat += 5f;
				if (Player.HasItem(ItemID.FrostDaggerfish)) damage.Flat += 5f;
			}
			if (item.type == ItemID.Katana) damage += .5f;
		}
	}
	public class ThrowingKnifeCustom : ModProjectile {
		public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.ThrowingKnife);
		public override void SetDefaults() {
			Projectile.width = 20;
		...
		public void Behavior(Player player, float offSet, float Counter, float Distance = 125) {
			...
			if (Counter == 0) for (int i = 0; i < 12; i++) {
					var randomSpeed = Main.rand.NextVector2Circular(1, 1);
	...
	public class ShurikenCustom : ModProjectile {
		public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.Shuriken);
		public override void SetDefaults() {
			Projectile.width = 20;
			Projectile.height = 20;
			Projectile.tileCollide = false;
